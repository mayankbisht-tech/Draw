 ...
 if (shape.type === 'pencil' && shape.points && shape.points.length > 0) {
  ...
  const hitTest = useCallback((x: number, y: number, shape: Shape, ctx: CanvasRenderingContext2D): boolean => {
    ...
    switch (shape.type) {
      ...
      case 'pencil': { if (!shape.points) return false; for (let i = 0; i < shape?.points?.length - 1; i++) { const p1 = shape.points[i]; const p2 = shape.points[i + 1]; const lenSq = (p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2; if (lenSq === 0) continue; let t = ((localMouse.x - p1.x) * (p2.x - p1.x) + (localMouse.y - p1.y) * (p2.y - p1.y)) / lenSq; t = Math.max(0, Math.min(1, t)); const dx = localMouse.x - (p1.x + t * (p2.x - p1.x)); const dy = localMouse.y - (p1.y + t * (p2.y - p1.y)); if ((dx * dx + dy * dy) <= tol * tol) return true; } return false; }
