...
export default function Imp() {
    ...
    const deleteShapeFromServer = useCallback(async (shapeId: string) => {
        if (!shapeId) return;
        try {
            await fetch(`${API_BASE_URL}/api/shape/${shapeId}`, {
                method: 'DELETE'
            });
        } catch (error) {
    ...
    const hitTest = useCallback((x: number, y: number, shape: Shape, ctx: CanvasRenderingContext2D): boolean => {
        ...
        switch (shape.type) {
            case 'rectangle':
                if (typeof shape.width !== 'number' || typeof shape.height !== 'number') return false;
                return localMouse.x >= shape.x - tol && localMouse.x <= shape.x + shape.width + tol &&
                       localMouse.y >= shape.y - tol && localMouse.y <= shape.y + shape.height + tol;
            case 'circle':
                if (typeof shape.radius !== 'number') return false;
                return Math.hypot(localMouse.x - shape.x, localMouse.y - shape.y) <= shape.radius + tol;
            case 'line': {
                if (typeof shape.x2 !== 'number' || typeof shape.y2 !== 'number') return false;
                const { x: x1, y: y1, x2, y2 } = shape;
                const dStart = Math.hypot(localMouse.x - x1, localMouse.y - y1);
                const dEnd = Math.hypot(localMouse.x - x2, localMouse.y - y2);
                if (dStart < tol || dEnd < tol) return true;

                const len = Math.hypot(x2 - x1, y2 - y1);
                if (len === 0) return dStart < tol;

                let t = ((localMouse.x - x1) * (x2 - x1) + (localMouse.y - y1) * (y2 - y1)) / (len * len);
                t = Math.max(0, Math.min(1, t));
                const closestX = x1 + t * (x2 - x1);
                const closestY = y1 + t * (y2 - y1);
                const distToLine = Math.hypot(localMouse.x - closestX, localMouse.y - closestY);
                return distToLine < tol;
            }
            case 'pencil': {
                if (!shape.points || !Array.isArray(shape.points) || shape.points.length < 2) return false;
                for (let i = 0; i < shape.points.length - 1; i++) {
                    const p1 = shape.points[i];
                    const p2 = shape.points[i + 1];
                    if (typeof p1.x !== 'number' || typeof p1.y !== 'number' || typeof p2.x !== 'number' || typeof p2.y !== 'number') continue;

                    const lenSq = (p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2;
                    if (lenSq === 0) continue;

                    let t = ((localMouse.x - p1.x) * (p2.x - p1.x) + (localMouse.y - p1.y) * (p2.y - p1.y)) / lenSq;
                    t = Math.max(0, Math.min(1, t));

                    const closestX = p1.x + t * (p2.x - p1.x);
                    const closestY = p1.y + t * (p2.y - p1.y);
                    const distToSegment = Math.hypot(localMouse.x - closestX, localMouse.y - closestY);
                    if (distToSegment <= tol) return true;
                }
                return false;
            }
            case 'text': {
                if (typeof shape.fontSize !== 'number' || typeof shape.text !== 'string') return false;
                const textWidth = ctx.measureText(shape.text).width;
                return (localMouse.x >= shape.x - tol && localMouse.x <= shape.x + textWidth + tol &&
                        localMouse.y >= shape.y - shape.fontSize - tol && localMouse.y <= shape.y + tol);
            }
            default:
                return false;
        }
    ...
    const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
        ...
        if (clickedShape) {
            if (selectedTool === 'eraser') {
                ...
                deleteShapeFromServer(clickedShape.id);
                return;
            }
            ...
            if ('touches' in e && e.touches.length === 2) {
                ...
                if (clickedShape.type === 'line' && clickedShape.x2 != null && clickedShape.y2 != null) {
                    ...
                    if (Math.hypot(localMouse.x - clickedShape.x, localMouse.y - clickedShape.y) < handleRadius) {
                        setLineDragHandle('start');
                    } else if (Math.hypot(localMouse.x - clickedShape.x2, localMouse.y - clickedShape.y2) < handleRadius) {
                        setLineDragHandle('end');
                    } else {
                        setLineDragHandle('body');
                    }
                }
            }
            return;
            ...
            setShapes(prev => prev.map(s => {
                if (s.id === selectedShapeId) {
                    ...
                    if ('touches' in e && e.touches.length === 2) {
                        ...
                        if (lineDragHandle === 'start') {
                            updatedShape.x = localMouse.x;
                            updatedShape.y = localMouse.y;
                        } else {
                            updatedShape.x2 = localMouse.x;
                            updatedShape.y2 = localMouse.y;
                        }
                    } else {
                        const coords = getCoords(e);
            ...
            setShapes(prev => prev.map(s => {
                if (s.id === currentDrawingShapeId) {
                    ...
                    switch (updatedShape.type) {
                        case 'pencil':
                            if (updatedShape.points && Array.isArray(updatedShape.points)) {
                                updatedShape.points.push(currentCoords);
                            } else {
                                updatedShape.points = [currentCoords];
                            }
                            break;
                        case 'rectangle':
                            updatedShape.x = Math.min(start.x, currentCoords.x);
                            updatedShape.y = Math.min(start.y, currentCoords.y);
                            updatedShape.width = Math.abs(start.x - currentCoords.x);
                            updatedShape.height = Math.abs(start.y - currentCoords.y);
                            break;
                        case 'circle':
                            updatedShape.radius = Math.hypot(currentCoords.x - start.x, currentCoords.y - start.y) / 2;
                            updatedShape.x = (start.x + currentCoords.x) / 2;
                            updatedShape.y = (start.y + currentCoords.y) / 2;
                            break;
                        case 'line':
                            updatedShape.x2 = currentCoords.x;
                            updatedShape.y2 = currentCoords.y;
                            break;
                        default:
                            break;
                    }
    ...
    }, [isDrawing, isTransforming, shapes, saveShapeToServer, currentDrawingShapeId, selectedShapeId]);
    ...
    const handleTextInputBlur = useCallback(() => {
        if (isTyping && textInputValue.trim() !== '' && textInputPosition && canvasRef.current) {
            ...
            const newTextShape: Shape = {
                id: newShapeId,
                type: 'text',
                x: textInputPosition.x - canvasRect.left,
                y: textInputPosition.y - canvasRect.top,
                text: textInputValue,
                fontSize: 24,
                fontFamily: 'Arial',
                color: 'white',
    ...
    const handleSendMessage = () => {
        ...
        if (ws.current?.readyState === WebSocket.OPEN) {
            ws.current.send(JSON.stringify({
                ...
                roomId: roomId,
                text: inputValue,
                senderFirstName: userInfo?.firstname || 'Guest',
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', background: '#333', color: 'white' }}>
            <Sidebar collapsed={collapsed} backgroundColor="#222" style={{ height: '100%', borderRight: '1px solid #444' }}>
                <Menu>
                    <MenuItem onClick={() => setCollapsed(!collapsed)} style={{ textAlign: 'center', padding: '10px 0' }}>
                        ...
                        {onlineUsers.length > 0 ? (
                            onlineUsers.map(user => (
                                <MenuItem key={user.userId}>
                                    {user.firstname} {user.lastname} {user.userId === userInfo?.userId ? '(You)' : ''}
                                </MenuItem>
                            ))
                        ) : (
                            <MenuItem disabled>No other users online</MenuItem>
                        )}
                    </SubMenu>
                </Menu>
            </Sidebar>
            ...
            <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', padding: '20px' }}>
                ...
                <div style={{ flexGrow: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', border: '1px solid #444', position: 'relative' }}>
                    <canvas
                        ...
                        style={{ background: '#282c34', display: 'block', border: '1px solid #555' }}
                    />
                ...
                <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', height: '200px', border: '1px solid #444', padding: '10px', background: '#222' }}>
                    <div style={{ flexGrow: 1, overflowY: 'auto', marginBottom: '10px', paddingRight: '10px' }}>
                        {messages.map((msg) => (
                            <div key={msg.id} style={{ marginBottom: '5px' }}>
                                <strong style={{ color: '#00bcd4' }}>{msg.senderFirstName}{msg.senderLastName ? ` ${msg.senderLastName}` : ''}:</strong> {msg.text}
                    ...
                    <div style={{ display: 'flex' }}>
                        ...
                        <button
                            ...
                            style={{
                                padding: '8px 15px',
                                background: wsConnected ? '#007bff' : '#6c757d',
                                color: 'white',
                                border: 'none',
                                borderRadius: '4px',
