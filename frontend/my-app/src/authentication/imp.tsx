...
const getShapeCenter = (shape: Shape): { x: number; y: number } => {
    let centerX = 'x' in shape ? shape.x : 0; let centerY = 'y' in shape ? shape.y : 0;
    ...
    else if (shape.type === 'pencil' && shape.points && Array.isArray(shape.points) && shape.points.length > 0) { const minX = Math.min(...shape.points.map(p => p.x)); const minY = Math.min(...shape.points.map(p => p.y)); const maxX = Math.max(...shape.points.map(p => p.x)); const maxY = Math.max(...shape.points.map(p => p.y)); centerX = minX + (maxX - minX) / 2; centerY = minY + (maxY - minY) / 2; }
    else if (shape.type === 'text' && shape.fontSize != null && shape.text != null) { const approxWidth = shape.text.length * (shape.fontSize * 0.5); centerX = shape.x + approxWidth / 2; centerY = shape.y - shape.fontSize / 2; }
    return { x: centerX, y: centerY };
};
const getLocalMouseCoordinates = (coords: { x: number; y: number }, shape: Shape, ctx: CanvasRenderingContext2D) => {
    ctx.save(); const { x: shapeCenterX, y: shapeCenterY } = getShapeCenter(shape); ctx.translate(shapeCenterX + (shape.offsetX || 0), shapeCenterY + (shape.offsetY || 0)); ctx.rotate(shape.rotation || 0); ctx.scale(shape.scale || 1, shape.scale || 1); ctx.translate(-shapeCenterX, -shapeCenterY); const invTransform = ctx.getTransform().inverse(); ctx.restore();
...
export default function Imp() {
        ...
        const connectWebSocket = () => {
            ...
            socket.onmessage = async (event) => {
                ...
                try {
                    ...
                    switch (data.type) {
                        case 'init': setShapes((data.shapes || []).map((shape: any) => ({ ...shape, offsetX: shape.offsetX ?? 0, offsetY: shape.offsetY ?? 0, rotation: shape.rotation ?? 0, scale: shape.scale ?? 1 }))); break;
                        case 'shape': setShapes((prev) => { const i = prev.findIndex(s => s.id === data.shape.id); if (i !== -1) { const u = [...prev]; u[i] = { ...data.shape, offsetX: data.shape.offsetX ?? 0, offsetY: data.shape.offsetY ?? 0, rotation: data.shape.rotation ?? 0, scale: data.shape.scale ?? 1 }; return u; } return [...prev, { ...data.shape, offsetX: data.shape.offsetX ?? 0, offsetY: data.shape.offsetY ?? 0, rotation: data.shape.rotation ?? 0, scale: data.shape.scale ?? 1 }]; }); break;
                        case 'delete': setShapes((prev) => prev.filter((s) => s.id !== data.id)); break;
                        case 'user_info': setUserInfo({ firstname: data.firstname, lastname: data.lastname, userId: data.userId }); break;
                        case 'online_users_update': setOnlineUsers(data.users || []); break;
                        case 'chat_message': setMessages((prev) => [...prev, data.message]); break;
                        default: console.warn("Unknown message type:", data.type);
                    }
                } catch (error) { console.error("Failed to parse message:", error, "Raw message:", messageData); }
            };
        };
        connectWebSocket();
    ...
    useEffect(() => { const canvas = canvasRef.current; if (!canvas || !canvas.getContext) return; const ctx = canvas.getContext("2d")!; ctx.clearRect(0, 0, canvas.width, canvas.height); shapes.forEach((shape) => { ctx.save(); const { x: centerX, y: centerY } = getShapeCenter(shape); ctx.translate(centerX + (shape.offsetX || 0), centerY + (shape.offsetY || 0)); ctx.rotate(shape.rotation || 0); ctx.scale(shape.scale || 1, shape.scale || 1); ctx.translate(-centerX, -centerY); ctx.strokeStyle = shape.id === selectedShapeId ? "cyan" : "white"; ctx.lineWidth = 2; try { if (shape.type === 'pencil' && shape.points && Array.isArray(shape.points) && shape.points.length > 0) { ctx.beginPath(); ctx.moveTo(shape.points[0].x, shape.points[0].y); for (let i = 1; i < shape.points.length; i++) { ctx.lineTo(shape.points[i].x, shape.points[i].y); } ctx.stroke(); } else if (shape.type === 'rectangle' && shape.width != null && shape.height != null) { ctx.strokeRect(shape.x, shape.y, shape.width, shape.height); } else if (shape.type === 'circle' && shape.radius != null) { ctx.beginPath(); ctx.arc(shape.x, shape.y, Math.abs(shape.radius), 0, 2 * Math.PI); ctx.stroke(); } else if (shape.type === 'line' && shape.x2 != null && shape.y2 != null) { ctx.beginPath(); ctx.moveTo(shape.x, shape.y); ctx.lineTo(shape.x2, shape.y2); ctx.stroke(); if (shape.id === selectedShapeId) { const r = 6 / (shape.scale || 1); ctx.fillStyle = "white"; ctx.beginPath(); ctx.arc(shape.x, shape.y, r, 0, 2 * Math.PI); ctx.fill(); ctx.beginPath(); ctx.arc(shape.x2, shape.y2, r, 0, 2 * Math.PI); ctx.fill(); } } else if (shape.type === 'text' && shape.fontSize != null && shape.text != null) { ctx.font = `${shape.fontSize}px ${shape.fontFamily || 'Arial'}`; ctx.fillStyle = shape.color || "white"; ctx.textBaseline = 'alphabetic'; ctx.fillText(shape.text, shape.x, shape.y); if (shape.id === selectedShapeId) { const metrics = ctx.measureText(shape.text); ctx.strokeStyle = "yellow"; ctx.lineWidth = 1; ctx.strokeRect(shape.x, shape.y - shape.fontSize, metrics.width, shape.fontSize); } } } catch (error) { console.error(`Error rendering shape ${shape.id}:`, error, shape); } ctx.restore(); }); }, [shapes, canvasDimensions, selectedShapeId]);
    useEffect(() => { const updateCanvasDimensions = () => { const sidebarWidth = collapsed ? 80 : 256; setCanvasDimensions({ width: window.innerWidth - sidebarWidth - 40, height: window.innerHeight - 80 - 40 }); }; updateCanvasDimensions(); window.addEventListener('resize', updateCanvasDimensions); return () => window.removeEventListener('resize', updateCanvasDimensions); }, [collapsed]);
    const saveShapeToServer = useCallback(async (shape: Shape) => { if (!roomId) return; try { await fetch(`${API_BASE_URL}/api/room/${roomId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(shape) }); } catch (error) { console.error("Failed to save shape:", error); } }, [roomId]);
    const deleteShapeFromServer = useCallback(async (shapeId: string) => { if (!shapeId) return; try { await fetch(`${API_BASE_URL}/api/room/shape/${shapeId}`, { method: 'DELETE' }); } catch (error) { console.error("Failed to delete shape from server:", error); } }, []);
    const broadcastData = useCallback((data: object) => { if (ws.current?.readyState === WebSocket.OPEN) { ws.current.send(JSON.stringify(data)); } else { console.warn("WebSocket not open. Message not sent:", data); } }, []);
    ...
    const hitTest = useCallback((x: number, y: number, shape: Shape, ctx: CanvasRenderingContext2D): boolean => { if (!shape) return false; const localMouse = getLocalMouseCoordinates({ x, y }, shape, ctx); const tol = 10 / (shape.scale || 1); switch (shape.type) { case 'rectangle': if (typeof shape.width !== 'number' || typeof shape.height !== 'number') return false; return localMouse.x >= shape.x - tol && localMouse.x <= shape.x + shape.width + tol && localMouse.y >= shape.y - tol && localMouse.y <= shape.y + shape.height + tol; case 'circle': if (typeof shape.radius !== 'number') return false; return Math.hypot(localMouse.x - shape.x, localMouse.y - shape.y) <= shape.radius + tol; case 'line': { if (typeof shape.x2 !== 'number' || typeof shape.y2 !== 'number') return false; const { x: x1, y: y1, x2, y2 } = shape; const dStart = Math.hypot(localMouse.x - x1, localMouse.y - y1); const dEnd = Math.hypot(localMouse.x - x2, localMouse.y - y2); if (dStart < tol || dEnd < tol) return true; const len = Math.hypot(x2 - x1, y2 - y1); if (len === 0) return dStart < tol; let t = ((localMouse.x - x1) * (x2 - x1) + (localMouse.y - y1) * (y2 - y1)) / (len * len); t = Math.max(0, Math.min(1, t)); const closestX = x1 + t * (x2 - x1); const closestY = y1 + t * (y2 - y1); const distToLine = Math.hypot(localMouse.x - closestX, localMouse.y - closestY); return distToLine < tol; } case 'pencil': { if (!shape.points || !Array.isArray(shape.points) || shape.points.length < 2) return false; for (let i = 0; i < shape.points.length - 1; i++) { const p1 = shape.points[i]; const p2 = shape.points[i + 1]; if (typeof p1.x !== 'number' || typeof p1.y !== 'number' || typeof p2.x !== 'number' || typeof p2.y !== 'number') continue; const lenSq = (p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2; if (lenSq === 0) continue; let t = ((localMouse.x - p1.x) * (p2.x - p1.x) + (localMouse.y - p1.y) * (p2.y - p1.y)) / lenSq; t = Math.max(0, Math.min(1, t)); const closestX = p1.x + t * (p2.x - p1.x); const closestY = p1.y + t * (p2.y - p1.y); const distToSegment = Math.hypot(localMouse.x - closestX, localMouse.y - closestY); if (distToSegment <= tol) return true; } return false; } case 'text': { if (typeof shape.fontSize !== 'number' || typeof shape.text !== 'string') return false; const textWidth = ctx.measureText(shape.text).width; return (localMouse.x >= shape.x - tol && localMouse.x <= shape.x + textWidth + tol && localMouse.y >= shape.y - shape.fontSize - tol && localMouse.y <= shape.y + tol); } default: return false; } }, []);
    ...
    const handleSendMessage = () => { if (inputValue.trim() === '' || !userInfo) return; const messagePayload = { type: 'chat_message', message: { id: generateId(), senderId: userInfo.userId, senderFirstName: userInfo.firstname, senderLastName: userInfo.lastname || '', text: inputValue, timestamp: Date.now() } }; if (ws.current?.readyState === WebSocket.OPEN) { ws.current.send(JSON.stringify(messagePayload)); setInputValue(''); } else { console.warn("WebSocket not open. Chat message not sent."); } };
    return (
        <div style={{ display: 'flex', height: '100vh', background: '#333', color: 'white' }}>
            <Sidebar collapsed={collapsed} backgroundColor="#222" style={{ height: '100%', borderRight: '1px solid #444' }}>
                <Menu>
                    <MenuItem onClick={() => setCollapsed(!collapsed)} style={{ textAlign: 'center', padding: '10px 0' }}>{collapsed ? 'Expand' : 'Collapse'}</MenuItem><hr style={{ borderColor: '#444' }} />
                    ...
                    <SubMenu label="Online Users">{onlineUsers.length > 0 ? (onlineUsers.map(user => (<MenuItem key={user.userId}>{user.firstname} {user.lastname} {user.userId === userInfo?.userId ? '(You)' : ''}</MenuItem>))) : (<MenuItem disabled>No other users online</MenuItem>)}</SubMenu>
                </Menu>
            </Sidebar>
